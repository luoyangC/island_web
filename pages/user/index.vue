        ...
        <v-card flat dark>
          <v-img :src="user.image" aspect-ratio="2">
            <v-layout column fill-height justify-center align-center dark-back>
              ...
              <v-layout justify-center align-center>
                <v-btn rounded outlined depressed large @click="logout">注销</v-btn>
                <v-btn rounded outlined depressed large @click="modify">修改</v-btn>
              </v-layout>
            </v-layout>
          </v-img>
        </v-card>
...
export default {
  ...
  methods: {
    async logout() {
      ...
      this.$store.commit('setCurrentUser', { id: null, image: 'https://luoyangc.oss-cn-shanghai.aliyuncs.com/media/image/icons/icon-a.png' })
      this.$router.go(-1)
    },
    async modify() {
      const data = {
